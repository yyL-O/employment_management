...
<html>
	...
	<body>
		<div class="wrapper">
			...
			<form id="updatePer" action="updateApp_ok.jsp" method="post">
				<div class="perDetaile">
					<h2><%=a.getCompanyName() %></h2>
				    <p><span>岗位名称</span><input type="text" name="jobName" value=<%=a.getJobName() %>></p>
					<p><span>工作地点</span><input type="text" name="jobPlace" value=<%=a.getJobPlace() %>></p>
					<p><span>每月薪资</span><input type="text" name="salary" value=<%=a.getSalary() %>></p>
					<p><span>岗位职责</span><textarea name="jobTask" ><%=a.getJobTask() %></textarea></p>
					<p><span>任职要求</span><textarea name="jobRequire"><%=a.getJobRequire() %></textarea></p>
